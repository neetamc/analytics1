...
sum(df$X); sum(df$Y)
sd(df$X) ; var(df$Y)
cov(df$X,df$Y); cov(df$Y,df$X)#covariance tells you only the direction of relationship
cor(df$X,df$Y) ; cor(df$Y,df$X)#correlation -1 to 0 to 10
#cor.test(df$X,df$Y)
...
fit1
summary(fit1)#when the f statistics p value is less than 0.05 the linear model exists and a there is atleast one x which has a relationship with y and we should be able to predict the relationship with y
#the r squared value = 0.9042 tells us that 90% of the variation in y is explained by x
#std deviation around the prediction line - residual error
# 3 stars means x is significant 
# pvalue for e ach coefficient ?
...
plot(fit1, which=1) 
plot(fit1)
par(mfrow = c(2,2)) # multiple frames per row 2x2
plot(fit1)
